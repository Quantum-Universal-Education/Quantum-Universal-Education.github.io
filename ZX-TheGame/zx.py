...
class Node:
    ...
    def __repr__(self):
        ...
        if self.type == 'Z_SPIDER':
            return f"Z({self.id}, φ={self.phase:.2f}π)" # Represent phase in terms of pi
        elif self.type == 'X_SPIDER':
            return f"X({self.id}, φ={self.phase:.2f}π)"
...
class QuantumGame:
    """
    Manages the game flow, user interaction, and win condition.
    """
    def __init__(self):
        self.diagram = ZXDiagram()
        self.setup_initial_diagram()
        self.max_moves = 5 # Set a reasonable limit for the tutorial game
        self.current_move = 0

    def setup_initial_diagram(self):
        """
        Sets up the starting ZX-diagram for the game.
        This diagram represents an identity operation, but is designed to be
        simplified using the Identity Rule.
        Diagram: Input -- Z(0) -- Output
        """
        self.node_w_in = self.diagram.add_node('WIRE_END') # Represents the input wire
        self.node_z = self.diagram.add_node('Z_SPIDER', 0.0) # A Z-spider with 0 phase
        self.node_w_out = self.diagram.add_node('WIRE_END') # Represents the output wire

        # Establish connections
        self.diagram.add_connection(self.node_w_in, self.node_z)
        self.diagram.add_connection(self.node_z, self.node_w_out)
        
        print("Initial Diagram Setup:")
    ...
    def check_win_condition(self):
        """
        Checks if the player has won the game (diagram simplified to a single wire).
        """
        ...
        if len(self.diagram.nodes) == 2:
            ...
            if nodes[0].type == 'WIRE_END' and nodes[1].type == 'WIRE_END':
                ...
                if nodes[0].id in nodes[1].connections:
                    print("\n⭐⭐⭐ Congratulations! You have simplified the diagram to a single wire! ⭐⭐⭐")
                    return True
        return False
    ...
    def run(self):
        ...
        print("Your goal is to simplify the given ZX-diagram using rewrite rules to a single wire.")
        print("Good luck!")
        ...
        while self.current_move < self.max_moves:
            ...
            print(f"\n--- Move {self.current_move + 1}/{self.max_moves} ---")
            ...
            try:
                ...
                if rule_choice == 1: # Spider Fusion
                    ...
                    if ZXRules.apply_hadamard_color_change(self.diagram, h_id, spider_id):
                        self.current_move += 1
                else:
        ...
        if not self.check_win_condition():
            print("\nGame over! You ran out of moves without simplifying the diagram completely.")
            self.diagram.display()
            print("Keep practicing to master ZX-calculus simplification!")

# Main execution block for the tutorial
