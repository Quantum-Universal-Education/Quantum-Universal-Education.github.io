...
def cirqbit(i : int) -> cirq.NamedQubit:
    return cirq.NamedQubit(str(i))
# ----------------------------------------------------------------------
class QubitAllocator(object):
    ...
    def measure_all(self, circuit: cirq.Circuit) -> cirq.Circuit:
        circuit.append(cirq.ops.measure(
            *[cirqbit(i) for i in self.allocated]))
        return circuit
...
def reset_all() -> None:
    ALLOCATOR.reset()
def measure_all(circuit: cirq.Circuit) -> cirq.Circuit:
    return ALLOCATOR.measure_all(circuit)
# ----------------------------------------------------------------------
class UnitaryOperation(object):
    ...
    def reverse(self) -> None:
        pass
    def synthesis(self, circuit : cirq.Circuit) -> cirq.Circuit:
        return circuit
...
class UO_H(UnitaryOperation):
    ...
    def reverse(self) -> None:
        pass
    def synthesis(self, circuit : cirq.Circuit) -> cirq.Circuit:
        circuit.append([cirq.H(cirqbit(self.q))])
        return circuit
    def string(self, depth: int) -> str:
...
class UO_X(UnitaryOperation):
    ...
    def reverse(self) -> None:
        pass
    def synthesis(self, circuit : cirq.Circuit) -> cirq.Circuit:
        circuit.append([cirq.X(cirqbit(self.q))])
        return circuit
    def string(self, depth: int) -> str:
...
class UO_CX(UnitaryOperation):
    ...
    def reverse(self) -> None:
        pass
    def synthesis(self, circuit: cirq.Circuit) -> cirq.Circuit:
        circuit.append([cirq.CX(cirqbit(self.c), cirqbit(self.x))])
        return circuit
    def string(self, depth: int) -> str:
...
class UO_CSWAP(UnitaryOperation):
    ...
    def synthesis(self, circuit: cirq.Circuit) -> cirq.Circuit:
        ...
        circuit.append([cirq.CCX(cirqbit(c), cirqbit(a), cirqbit(b)),
                        cirq.CCX(cirqbit(c), cirqbit(b), cirqbit(a)),
                        cirq.CCX(cirqbit(c), cirqbit(a), cirqbit(b))])
        return circuit
    def string(self, depth: int) -> str:
...
class Carry(UnitaryOperation):
    ...
    def synthesis(self, circuit: cirq.Circuit) -> cirq.Circuit:
        ...
        if not self.is_reversed:
            circuit.append([cirq.CCX(cirqbit(b), cirqbit(c), cirqbit(d)),
                            cirq.CX(cirqbit(b), cirqbit(c)),
                            cirq.CCX(cirqbit(a), cirqbit(c), cirqbit(d))])
        else:
            circuit.append([cirq.CCX(cirqbit(a), cirqbit(c), cirqbit(d)),
                            cirq.CX(cirqbit(b), cirqbit(c)),
                            cirq.CCX(cirqbit(b), cirqbit(c), cirqbit(d))])
        return circuit
    def string(self, depth: int) -> str:
        return (
...
class Sum(UnitaryOperation):
    ...
    def synthesis(self, circuit: cirq.Circuit) -> cirq.Circuit:
        ...
        if not self.is_reversed:
            circuit.append([cirq.CX(cirqbit(b), cirqbit(c)),
                            cirq.CX(cirqbit(a), cirqbit(c))])
        else:
            circuit.append([cirq.CX(cirqbit(a), cirqbit(c)),
                            cirq.CX(cirqbit(b), cirqbit(c))])
        return circuit
    def string(self, depth: int) -> str:
        return (
...
with TestProc_ADD() as p_add:
    ...
    print('b =' + str(p_add.b))
    print('---- simulator ----')
    circuit = p_add.synthesis(cirq.Circuit())
    #p_add.a.measure(circuit)
    #p_add.b0.measure(circuit)
    #p_add.b.measure(circuit)
    measure_all(circuit)
    print(str(circuit))
    print('---- result ----')
    start = time.perf_counter()
    for _ in range(10):
        result = str(cirq.Simulator().simulate(circuit))
...
with TestProc_SUB() as p_sub:
    ...
    print('b =' + str(p_sub.b))
    print('---- simulator ----')
    circuit = p_sub.synthesis(cirq.Circuit())
    #p_sub.a.measure(circuit)
    #p_sub.b0.measure(circuit)
    #p_sub.b.measure(circuit)
    measure_all(circuit)
    print(str(circuit))
    print('---- result ----')
    start = time.perf_counter()
...
with TestMul() as p_mul:
    ...
    print('c =' + str(p_mul.c))
    print('---- simulator ----')
    circuit = p_mul.synthesis(cirq.Circuit())
    measure_all(circuit)
    print(str(circuit))
    print('---- result ----')
    start = time.perf_counter()
