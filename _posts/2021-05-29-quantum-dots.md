...
Mainly 1) These nanostructures are so tightly confined that they are said to be “0D”
...
Another way we can go about making a QD is by synthesizing Metallic particles with Chemistry
	- Actually kind of cool cause of how quick (check out youtube vids)
...
Using an oscillating magnetic or electric fields to drive transitions from spin up to spin down 

Or we can take a more indirect approach by letting the interactions between electrons themselves act as the manipulation:
Couple two electron qubit wavefunctions
...
Now this is kind of tricky because we can’t actually directly measure the spin of an electron 
	Well, we can, but its really impractical, especially with this application.
...
<br>
First Result After we’ve done some manipulation we basically do the reverse of initialization
...
<br>
Second Result After we’ve done some manipulation we basically do the reverse of initialization
...
However, comparing to superconducting qubit, QD qubit is more noisy. Since superconducting qubit use relatively macroscopic phenomena (joseph junction); while QD is only an microscopic electron. It’s hard to construct multi qubit QD device and maintain the fidelity at the same time.
